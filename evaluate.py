...
for trial_idx in range(1,len(clearance_log)):
    ...
    if len(tmp_executed_action_log) > 0:
    ...
    if len(tmp_grasp_attempt_ind) > 0:
        grasp_success_rate[trial_idx-1] = float(tmp_num_grasp_success)/float(len(tmp_grasp_attempt_ind))
    
    if len(tmp_executed_action_log) == 0 or len(tmp_grasp_attempt_ind) == 0:
        nonvalid_clearance += 1


# Which trials reached task completion?
valid_clearance = grasp_num_success >= num_obj_complete
print(valid_clearance)
print(max_trials-nonvalid_clearance)


# Display results
print('Average %% clearance: %2.1f' % (float(np.sum(valid_clearance))/float(max_trials-nonvalid_clearance)*100))
...
print('Averafe %% motion number: %2.2f' % (np.mean(num_actions_before_completion[valid_clearance])))
